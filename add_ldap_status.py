...
class AddLdapStatusParser(envvars.ArgumentParser):
    ...
    @staticmethod
    def create():
        description = "add columns 'status', 'name' and 'org_unit' and fill with data retrived from fldap"
        ...
        parser.add_argument("--creatorId", "-c",  default='creator_id', help="name of column that contains netid/email")
        parser.add_argument("--entryId", "-e",  default='entry_id', help="name of column that contains teh kaltura entry id")
        return parser
...
def _enhance(tsvin, entryid_col, netid_col, ldap):
    ...
    for line in tsvin.readlines():
        ...
        _do_entry(ldap, eid, netid, line)


def _do_entry(ldap, eid, netid, line):
